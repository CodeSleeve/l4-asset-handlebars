...
namespace Codesleeve\L4AssetHandlebars\Filters;
...
class HandlebarsFilter implements FilterInterface
{
     ...
     * @return {[type]}       [description]
     */
    protected function strip_off_extension($path, $prefix)
    {
        $prefix_size = strlen($prefix);
        $path_size = strlen($path);

        if (substr($path, $path_size - $prefix_size, $prefix_size) == $prefix) {
            $path = substr($path, 0, $path_size - $prefix_size);
        }
